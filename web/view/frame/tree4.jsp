...
<body style="width:100%;height:100%;">
	<ul id="tree" class="filetree"
		...
		<li><span class="folder fontcolor" 
			onclick=" window.parent.selectInfo4('${id}','${departmentName}');">${departmentName}</span>
			<ul class="filetree">
				<c:forEach var="dept" items="${data}" varStatus="st">
					<li><span class="folder fontcolor"
						onclick="window.parent.selectInfo4('${dept.id}','${dept.departmentNameZh }');">${dept.departmentNameZh}</span>
						<c:if test="${dept.list!=null}">
							<ul>
								<c:forEach var="dept2" items="${dept.list}">
									<li><span class="folder fontcolor"
										onclick="window.parent.selectInfo4('${dept2.id}','${dept2.departmentNameZh }');">${dept2.departmentNameZh}</span>
										<c:if test="${dept2.list!=null}">
											<ul>
												<c:forEach var="dept3" items="${dept2.list}">
													<li><span class="folder fontcolor"
														onclick="window.parent.selectInfo4('${dept3.id}','${dept3.departmentNameZh }');">${dept3.departmentNameZh}</span>
														<c:if test="${dept3.list!=null}">
															<ul>
																<c:forEach var="dept4" items="${dept3.list}">
																	<li><span class="folder fontcolor"
																		onclick="window.parent.selectInfo4('${dept4.id}','${dept4.departmentNameZh }');">${dept4.departmentNameZh}</span>
																		<c:if test="${dept4.list!=null}">
																			<ul>
																				<c:forEach var="dept5" items="${dept4.list}">
																					<li><span class="folder fontcolor"
																						onclick="window.parent.selectInfo4('${dept5.id}','${dept5.departmentNameZh }');">${dept5.departmentNameZh}</span>
																				</c:forEach>
																			</ul>
																		</c:if></li>
																</c:forEach>
